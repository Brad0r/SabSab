...
VIDEO_PATH = Path(r"C:\Users\bradl\Downloads\2026-04-07 16-30-22.mp4")
OUT_DIR = Path(r"c:\Users\bradl\Desktop\Codage\SabSab\assets\instruments")
TEMP_AUDIO = OUT_DIR / "_panflute_video_source.wav"
OUTPUT_INFO = OUT_DIR / "panflute-video.json"
TARGET_SR = 44100
TARGET_NOTE_IDS = [
    "c3", "d3", "e3", "f3", "g3",
    "a3", "b3", "c4", "d4", "e4",
...
clip.audio.write_audiofile(str(TEMP_AUDIO), fps=TARGET_SR, nbytes=2, logger=None)
clip.close()
...
for index in range(len(boundaries) - 1):
    ...
    f0 = librosa.yin(segment, fmin=80, fmax=2200, sr=sr, frame_length=2048, hop_length=256)
...
for item in segments:
    ...
    prev = merged[-1]
    if item["note"] == prev["note"] and ((item["start"] - prev["end"]) / sr) < 0.12:
        prev["end"] = item["end"]
        prev["duration"] = (prev["end"] - prev["start"]) / sr
        prev["rms"] = max(prev["rms"], item["rms"])
        prev["median_freq"] = float((prev["median_freq"] + item["median_freq"]) / 2)
    else:
        merged.append(item.copy())

if len(merged) < len(TARGET_NOTE_IDS):
    raise SystemExit(f"Only {len(merged)} merged notes found; expected {len(TARGET_NOTE_IDS)}.")

selected = merged[: len(TARGET_NOTE_IDS)]
pad = int(sr * 0.02)
...
for target_note_id, item in zip(TARGET_NOTE_IDS, selected):
    ...
    output_path = OUT_DIR / f"panflute-held-{target_note_id}.wav"
...
OUTPUT_INFO.write_text(json.dumps({
